...
const Index = () => {
  ...
  const latestFrequency = timeSeriesInsights?.length
    ? (
        Number.parseInt(timeSeriesInsights[timeSeriesInsights.length - 1].impressions) /
        Number.parseInt(timeSeriesInsights[timeSeriesInsights.length - 1].reach)
      ).toFixed(2)
    : 0
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-950 dark:to-slate-950">
      {/* Header Section */}
      <header className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border-b border-gray-200 dark:border-gray-700 sticky top-0 z-50 shadow-sm">
        <div className="container mx-auto py-6 px-4">
          <div className="flex flex-col space-y-4 md:flex-row md:items-center md:justify-between md:space-y-0">
            <div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {timeSeriesInsights?.length > 0 && (
            <>
              <CampaignChart data={timeSeriesInsights} title="Daily Ad Spend" metric="spend" />
              <BarChartVertical data={timeSeriesInsights} title="Daily Impressions" dataKey="impressions" />
            </>
          )}
        </div>
        ...
        {(timeSeriesError || aggregatedError) && (
