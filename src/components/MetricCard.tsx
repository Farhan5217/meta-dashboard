...
//   // Transform to difference data (changes between days)
//   const differenceData = transformToDifferenceData(chartData);
...
//                         const isFirstOrLast = index === 0 || index === differenceData.length - 1;
                        
//                         if (!isFirstOrLast) return null;
                        
//                         const dotSize = index === differenceData.length - 1 ? 4 : 3;
//                         const strokeWidth = index === differenceData.length - 1 ? 1.5 : 1;
...
// // Function to transform time series data to difference data
// function transformToDifferenceData(chartData: { date: string; value: number }[]) {
...
//   trend,
//   variant = "default",
//   chartData = [], // Default to empty array instead of dummy data
// }: MetricCardProps) {
//   // Get colors based on variant
//   const colors = getChartColors(variant, trend?.isPositive)
...
//   const hasChartData = differenceData && differenceData.length > 0;
...
//               {/* Chart container with Recharts - only show if data is available */}
//               {hasChartData ? (
//                 <div className="mt-4 h-12 w-full">
//                   <ResponsiveContainer width="100%" height={50}>
//                     <AreaChart
